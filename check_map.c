...
/*   Updated: 2023/02/16 18:55:48 by aasselma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_isvalid(int w, char *str, int i)
{
	if (w == 0)
	{
		...
		{
			ft_printf("error in line %d in index %d", w + 1, i + 1);
			exit(1);
		}
	}
	if (w != 0)
	{
		...
		while (str[i] != '\n' && str[i] != '\0')
		{
			...
			{
				ft_printf("error in line %d index %i", w + 1, i + 1);
				exit(1);
			}
			i++;
		}
...
void	check_map(int fd, int *w, int *I)
{
	...
	while (1)
	{
		...
		if (ft_strlen_t(str) != *I)
		{
			ft_printf("error line %d and line %d are not equals", *w + 1, *w);
			exit(1);
		}
